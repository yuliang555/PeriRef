...
if __name__ == '__main__':
    ...
    for dataset in DATASET_LIST:
        ...
        for seed in SEED_LIST:
            ...
            Ns, Nl = len(cfg['short_periods']), len(cfg['long_periods'])
            P_max, P_min = max(cfg['long_periods']), min(cfg['short_periods'])
            random.seed(seed)
            periods = [round(exp(random.uniform(log(P_min), log(P_max)))) for _ in range(Ns + Nl)]
            periods = sorted(periods, reverse=True)
            cfg['short_periods'] = periods[Nl:]
            cfg['long_periods'] = periods[:Nl]         
