...
# basic config
parser.add_argument('--is_training', type=int, default=2, help='status')
...
parser.add_argument('--ref_wave', type=str, default="sincos", help='reference series type, options:[sincos, sin, swatooth, reactangle, pulse]')
parser.add_argument('--ref_len', type=int, default=96, help="reference series length")
parser.add_argument('--use_norm', type=int, default=1, help='1: use revin or 0: no revin')
parser.add_argument('--use_pos', type=int, default=0, help='1: use position, 0: no position')
...
if args.is_training == 3:
    print(f'>>>>>>>Seasonal Climatology : {args.model_id}>>>>>>>>>>>>>>>>>>>>>>>>>>')
    exp.seasonal_climatology()
elif args.is_training == 2:
    print(f'>>>>>>>Harmonic Regression : {args.model_id}>>>>>>>>>>>>>>>>>>>>>>>>>>')
    exp.harmonic_regression()
else:
    if args.is_training == 1:
        setting = '{}_{}_sl={}_pl={}_seed={}'.format(            
            args.model,
            args.model_id,
            args.seq_len,                      
            args.pred_len,
