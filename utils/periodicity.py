...
class Top_Draw:
    ...
    def plot_channel(self, channel: str, acf: np.ndarray) -> Path:
        ...
        # Use an installed TrueType serif font so the PDF does not contain Type 3 text.
        with plt.rc_context({
            "font.family": "serif",
            ...
            if year_lags:
                right_limit = max(
                    right_limit,
                    year_lags[-1] + self.annual_period * 0.18,
                )
            ax.set_xlim(-0.5, right_limit)
            ax.set_xlabel("")
            ax.set_ylabel("", labelpad=0, fontsize=15)

            # Keep the minimum, zero, and one as the three y-axis ticks.
            minimum_value = float(np.nanmin(acf))
            ax.set_ylim(bottom=minimum_value, top=1.0)
            ax.yaxis.set_major_locator(
                FixedLocator([minimum_value, 0.0, 1.0])
            )
            ax.yaxis.set_major_formatter(FormatStrFormatter("%.1f"))

            displayed_year_lags = year_lags[:1]
            ax.set_xticks(displayed_year_lags)
            ax.set_xticklabels(
                ["1-year lag"] if displayed_year_lags else [],
                fontsize=20,
            )
            ax.tick_params(axis="x", which="minor", bottom=False)
            ax.tick_params(axis="both", which="major", pad=3, labelsize=36)
            ax.grid(True, axis="y", alpha=0.22, linewidth=0.55)
            ax.grid(False, axis="x")
            ax.margins(x=0.0, y=0.035)
            fig.subplots_adjust(left=0.16, right=0.96, bottom=0.25, top=0.93)

            out_dir = self.out_dir
            out_dir.mkdir(parents=True, exist_ok=True)
            out_path = self.out_dir / f"ACF_{self.dataset}_{safe_filename(channel)}.pdf"
